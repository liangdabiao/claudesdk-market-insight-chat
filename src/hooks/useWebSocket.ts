    ...
    ws.onmessage = (event) => {
      ...
      switch (data.type) {
        ...
        case "assistant_message":
          ...
          setMessages((prev) => [
            ...prev,
            { id: crypto.randomUUID(), role: "assistant", content: data.content || "", timestamp: Date.now() },
          ]);
          break;
        ...
        case "tool_use":
          setMessages((prev) => [
            ...prev,
            {
              ...
              role: "system",
              content: "",
              toolCall: { name: data.toolName || "unknown", input: data.toolInput, status: "running" },
              timestamp: Date.now(),
            },
          ]);
          break;
        ...
        case "error":
          ...
          setMessages((prev) => [
            ...prev,
            { id: crypto.randomUUID(), role: "system", content: `错误: ${data.error || "未知错误"}`, timestamp: Date.now() },
          ]);
          break;
      }
    };
