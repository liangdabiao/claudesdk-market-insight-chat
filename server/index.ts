...
const REPORT_DIR = path.resolve(process.cwd(), "insight_reports");

[UPLOAD_DIR, REPORT_DIR].forEach((d) => {
...
app.use("/insight_reports", express.static(REPORT_DIR));
...
app.get("/api/reports", (_req, res) => {
  const tree = scanDir(REPORT_DIR, REPORT_DIR);
  res.json(tree);
});
...
server.listen(PORT, () => {
  fileLog("Server", `MarketInsightChat Server listening on :${PORT}`);
});
